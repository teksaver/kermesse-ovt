...
- **Transfert de Données (DTO vs Arrays)** : **Contrainte stricte :** L'utilisation de DTOs (Data Transfer Objects) via des classes `readonly` (PHP 8.2) est **obligatoire** pour transférer des données d'un Contrôleur vers un Service. Il est **strictement interdit** de passer des tableaux associatifs en vrac ou une longue liste de variables scalaires pour des opérations d'écriture. Le Contrôleur valide les inputs, instancie un DTO fortement typé, puis le transmet à la couche Service.
