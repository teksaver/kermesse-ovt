<?php
$safeValidationUrl = filter_var($validationUrl ?? '', FILTER_VALIDATE_URL) ? (string) $validationUrl : '';
$validationScheme = strtolower((string) parse_url($safeValidationUrl, PHP_URL_SCHEME));

if ($safeValidationUrl === '' || ! in_array($validationScheme, ['http', 'https'], true)) {
    throw new InvalidArgumentException('A valid absolute validation URL is required.');
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <title>Validez votre kermesse</title>
</head>
...
<tr><td align="center" style="padding:32px 16px;">
    ...
    <tr><td style="padding:32px 24px;">
        <h1 style="margin:0 0 16px; font-size:24px; font-weight:700; color:#111827;">
            Bienvenue, <?= esc($ownerName) ?> !
        </h1>
        <p style="margin:0 0 16px; color:#475569;">
            Votre kermesse <strong>« <?= esc($kermesseName) ?> »</strong> a été créée.
            Cliquez sur le bouton ci-dessous pour valider votre email et accéder
            à l'espace d'administration.
        </p>
		        <p style="margin:0 0 24px; text-align:center;">
	            <a href="<?= esc($safeValidationUrl, 'attr') ?>"
        ...
        <p style="margin:0 0 8px; color:#475569; font-size:14px;">
            Si le bouton ne fonctionne pas, copiez ce lien dans votre navigateur :
		        ...
		        <p style="margin:0 0 16px; word-break:break-all; color:#2563EB; font-size:14px;">
		            <?= esc($safeValidationUrl) ?>
		        </p>
        <hr style="border:none; border-top:1px solid #CBD5E1; margin:24px 0;">
        <p style="margin:0; color:#475569; font-size:13px;">
            Ce lien expire dans 24 heures. Si vous n'avez pas créé cette kermesse,
