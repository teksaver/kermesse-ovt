...
<main class="app-shell">
    <section class="result-panel" aria-labelledby="page-title">
        <?php
        // Validate loginUrl: must be an absolute http/https URL (prevents javascript: injection).
        // Fall back to site_url if empty or untrusted, then HTML-escape for attribute context.
        $resolvedLoginUrl = (! empty($loginUrl) && preg_match('/^https?:\/\//i', $loginUrl))
            ? $loginUrl
            : site_url('owner/login');
        $safeLoginHref = esc($resolvedLoginUrl);
        ?>
        ...
        <a href="<?= $safeLoginHref ?>" class="btn btn-primary">Me connecter</a>
        ...
        <a href="<?= $safeLoginHref ?>" class="btn btn-primary">Demander un nouveau lien</a>
        ...
        <a href="<?= $safeLoginHref ?>" class="btn btn-primary">Retour à la connexion</a>
