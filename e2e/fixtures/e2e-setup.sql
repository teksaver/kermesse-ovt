...
-- ----------------------------------------------------------------
INSERT INTO `slot_signups` (`slot_id`, `user_id`, `created_by`, `accepted_at`) VALUES
...
INSERT INTO `slot_signups` (`slot_id`, `user_id`, `created_by`, `first_name`, `last_name`, `email`)
VALUES
...
-- ----------------------------------------------------------------
-- Story 6.5 — Fixtures for organizer E2E tests
-- ----------------------------------------------------------------

-- Pending invitation: invited but never accepted (invited_at IS NOT NULL, accepted_at IS NULL,
-- first_access_at IS NULL) — used to test the "relancer l'invitation" (resend) flow.
INSERT INTO `users` (`email`, `email_hash`, `first_name`, `last_name`, `phone`) VALUES
  ('pending-invite@e2e.test', SHA2('pending-invite@e2e.test', 256), 'Pending', 'Invite', '');
SET @pending_id = (SELECT `id` FROM `users` WHERE `email` = 'pending-invite@e2e.test');

INSERT INTO `kermesse_user_roles`
  (`kermesse_id`, `user_id`, `role`, `invited_by`, `invited_at`, `accepted_at`, `first_access_at`)
VALUES
  (@kermesse_id, @pending_id, 'gestionnaire', @owner_id, NOW(), NULL, NULL);

-- Kermesse lifecycle: starts in 'preparation' state with a stand + slot so it can be opened.
-- Used exclusively for the open/close lifecycle E2E test.
INSERT INTO `kermesses`
  (`created_by`, `public_slug`, `name`, `event_date`, `location`, `short_description`, `status`)
VALUES
  (@owner_id, 'kermesse-e2e-lifecycle', 'Kermesse Lifecycle E2E', @event_date,
   'Salle lifecycle', 'Kermesse pour tests lifecycle', 'preparation');
SET @kermesse_lifecycle_id = LAST_INSERT_ID();
INSERT INTO `kermesse_user_roles`
  (`kermesse_id`, `user_id`, `role`, `invited_by`, `accepted_at`, `first_access_at`)
VALUES
  (@kermesse_lifecycle_id, @owner_id, 'owner', NULL, NOW(), NOW());
INSERT INTO `stands` (`kermesse_id`, `name`, `display_order`) VALUES
  (@kermesse_lifecycle_id, 'Stand Lifecycle E2E', 1);
SET @stand_lifecycle_id = LAST_INSERT_ID();
INSERT INTO `slots` (`stand_id`, `starts_at`, `ends_at`, `capacity`) VALUES
  (@stand_lifecycle_id, @event_date + INTERVAL 10 HOUR, @event_date + INTERVAL 12 HOUR, 5);

-- Stand Organisateurs E2E: dedicated stand with isolated slots for organizer CRUD tests.
-- All slots use times that don't conflict with other@e2e.test's only existing signup (7:00-9:00).
INSERT INTO `stands` (`kermesse_id`, `name`, `display_order`) VALUES
  (@kermesse_id, 'Stand Organisateurs E2E', 8);
SET @org_stand_id = LAST_INSERT_ID();

-- Slot 1 (16:00–17:00): admin cancel test — pre-seeded self-signed signup for other@e2e.test
INSERT INTO `slots` (`stand_id`, `starts_at`, `ends_at`, `capacity`) VALUES
  (@org_stand_id, @event_date + INTERVAL 16 HOUR, @event_date + INTERVAL 17 HOUR, 5);
SET @slot_org_cancel = LAST_INSERT_ID();
INSERT INTO `slot_signups` (`slot_id`, `user_id`, `created_by`, `accepted_at`) VALUES
  (@slot_org_cancel, @other_id, @other_id, NOW());

-- Slot 2 (17:00–18:00): admin add test — empty slot (fresh volunteers added here)
INSERT INTO `slots` (`stand_id`, `starts_at`, `ends_at`, `capacity`) VALUES
  (@org_stand_id, @event_date + INTERVAL 17 HOUR, @event_date + INTERVAL 18 HOUR, 5);

-- Slot 3 (19:00–20:00): admin move-source test — pre-seeded self-signed signup for other@e2e.test
INSERT INTO `slots` (`stand_id`, `starts_at`, `ends_at`, `capacity`) VALUES
  (@org_stand_id, @event_date + INTERVAL 19 HOUR, @event_date + INTERVAL 20 HOUR, 5);
SET @slot_org_move_src = LAST_INSERT_ID();
INSERT INTO `slot_signups` (`slot_id`, `user_id`, `created_by`, `accepted_at`) VALUES
  (@slot_org_move_src, @other_id, @other_id, NOW());

-- Slot 4 (22:00–23:00): admin move-target test — empty (capacity 5); always a valid target
-- because 22:00-23:00 does not overlap with other@e2e.test's other signups (7:00-9:00, 16:00-17:00,
